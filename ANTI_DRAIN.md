...
- **44/44 unit tests** ([`apps/sponsor/src/test-antidrain.ts`](apps/sponsor/src/test-antidrain.ts)) — **18 cover the claim policy**: 4 legitimate shapes accepted, 14 drain vectors rejected (sponsor-sourced payment/changeTrust, `startingBalance > 0`, wrong asset, wrong `balanceId`, wrong tx source, too many ops, wrong destinations, a third-party-sourced op, and the strict-mode fail-closed cases). The remaining **7 cover the onward-send policy** (see the extension note below). The tests import the **same** module the live `/feebump` uses.
